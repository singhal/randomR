files <- list.files("/media/DataDrive/sutureGenomics/",recursive=T,pattern="errorChecking",full.names=T)
...
for (i in 1:length(files)) {
	...
	name <- sub("/errorChecking.out", "",file)
...
write.table(error,file="errorProfile.out",row.names=F,col.names=F,sep="\t")
